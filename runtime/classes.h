...
//
// `$name` must be prefixed with `type_` in order to avoid conflicts with C keywords and/or
// macros. This macro will also define a global with the name `$name` that points to the
// type object.
#define DEFINE_TYPE($name, $intrinsic)                          \
    extern vector_t(symbol_t) py_##$name##_attrs;               \  
    const pyobj_t py_##$name = {                                \   
        .type = &py_type_type,                                  \
        .as_type = {                                            \
            .class_attributes = &py_##$name##_attrs,            \
            .is_intrinsic = $intrinsic,                         \
            .base = NULL                                        \
        }                                                       \
    };                                                          \
    const pyobj_t* KNOWN_GLOBAL($name) = &py_##$name;           \
...
#define CLASS_ATTRIBUTES_BEGIN($name)                   \
    symbol_t py_##$name##_attrs_initial[] = {

// Specifies that a type method should be included in a type attribute list defined
// by `CLASS_ATTRIBUTES_BEGIN`.
#define HAS_CLASS_METHOD($type, $name)                                   \
    { .name = #$name, .value = &py_##$type##_method_##$name }

// Defines the implementation of the type method `$name`, belonging to type `$type`.
#define CLASS_METHOD($type, $name)                                      \
    PY_DEFINE(py_##$type##_method_##$name##_fn);                        \
    static const pyobj_t py_##$type##_method_##$name = {                \
        .type = &py_type_callable,                                      \
        .as_callable = &py_##$type##_method_##$name##_fn                \
    };                                                                  \
    PY_DEFINE(py_##$type##_method_##$name##_fn)                         

// Ends a class attribute list. This will define a `py_type_<name>_attrs` symbol that
// represents the attribute table for type `$name`.
#define CLASS_ATTRIBUTES_END($name)                                           \
    };                                                                        \   
    vector_t(symbol_t) py_##$name##_attrs = {                                 \
        .elements = &py_##$name##_attrs_initial,                              \
        .length = sizeof(py_##$name##_attrs_initial) / sizeof(symbol_t),      \
        .capacity = sizeof(py_##$name##_attrs_initial) / sizeof(symbol_t)     \
    };                                                                        \
