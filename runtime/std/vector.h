...
typedef struct vector_any { VECTOR_T_FIELDS(void) } vector_any_t;

// Declares that the given translation unit will use a `vector_t(T)` structure.
#define USES_VECTOR_FOR(T) struct vector_##T { VECTOR_T_FIELDS(T) }

// Represents a vector that holds elements of type `T`.
// Usage:
// ```
//      vector_t(int) vec = {};
//      std_vector_append(&vec, 1);
//      std_vector_append(&vec, 2);
...
#define std_vector_append($vector, $element)                                         \
    {                                                                                \
        __typeof__(($element)) element = ($element);                                 \
        std_vector_any_append(                                                       \
            (vector_any_t*)($vector),                                                \
...
#define std_vector_unit_size($vector) sizeof(*($vector)->elements)

// Removes the element at the given index.
#define std_vector_remove($vector, $index)  \
    std_vector_any_remove((vector_any_t*)($vector), $index, std_vector_unit_size($vector))
...
// function, and should be avoided if possible. See `std_vector_append` instead.
...
// function, and should be avoided if possible. See `std_vector_remove` instead.
